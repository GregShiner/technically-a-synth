...
mod app {
    ...
    fn fill_buffer_stereo(buf: &mut SampleBuffer, osc: &mut Sine<ConstHz>) {
        fn f64_to_sample(s: f64) -> AudioSample {
            (s * AudioSample::MAX as f64).clamp(AudioSample::MIN as f64, AudioSample::MAX as f64)
                as AudioSample
        }
        ...
        for i in (0..buf.len()).step_by(2) {
            let next = f64_to_sample(osc.next());
            buf[i] = next;
            buf[i + 1] = next;
        }
    }

    #[task(local = [square_osc, i2s2, buf: SampleBuffer = [0; BUFFER_SAMPLES]], priority = 1)]
    async fn write_osc_to_i2s(mut cx: write_osc_to_i2s::Context) -> ! {
        ...
        debug!("i2s2 Started");
        loop {
            fill_buffer_stereo(cx.local.buf, cx.local.square_osc);
